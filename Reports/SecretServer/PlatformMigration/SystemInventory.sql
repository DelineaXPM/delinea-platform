...
UNION ALL
SELECT 'Report Version', '1.2.20240917', ''
UNION ALL
...
UNION ALL
SELECT 'Number of Web Servers', CAST(COUNT(NodeId) AS NVARCHAR(50)), ''
FROM tbNode n
UNION ALL
SELECT 'Number of Domains', CAST(COUNT(DomainId) AS NVARCHAR(50)), ''
FROM tbDomain d
WHERE d.Active = 1
UNION ALL
SELECT 'Integrated Windows Auth', 
    CAST(CASE 
...
UNION ALL
SELECT 'Number of Users', CAST(COUNT(UserId) AS NVARCHAR(50)), ''
FROM tbUser u
WHERE u.Enabled = 1
UNION ALL
SELECT 'Number of Domain Users', CAST(COUNT(UserId) AS NVARCHAR(50)), ''
FROM tbUser u
WHERE u.Enabled = 1 AND u.DomainId IS NOT NULL
UNION ALL
SELECT 'Number of Local Users', CAST(COUNT(UserId) AS NVARCHAR(50)), ''
FROM tbUser u
WHERE u.Enabled = 1 AND u.DomainId IS NULL
UNION ALL
SELECT 'Number of Application Accounts', CAST(COUNT(UserId) AS NVARCHAR(50)), ''
FROM tbUser u
...
WHERE s.Active = 1
UNION ALL
SELECT 'Duplicate Secret Count', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM (
    SELECT s.SecretID
    FROM tbSecret s
    JOIN (
...
UNION ALL
SELECT 'Secrets Without Folders', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbSecret s
WHERE s.FolderId IS NULL AND s.Active = 1
UNION ALL
SELECT 'Secrets Without Owners', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbSecret s
...
UNION ALL
SELECT 'Secrets Using Inactive Templates', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbSecret s
...
UNION ALL
SELECT 'Secrets in Inactive Personal Folders', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbSecret s
JOIN tbFolder f ON s.FolderId = f.FolderID
...
UNION ALL
SELECT 'Active Secrets With Files', CAST(COUNT(*) AS NVARCHAR(50)), ''
...
FROM tbDoubleLock
UNION ALL
SELECT 'Folders with Leading or Trailing Spaces', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbfolder f
WHERE f.FolderName LIKE ' %' OR f.FolderName LIKE '% '
UNION ALL
SELECT 'Secrets with Leading or Trailing Spaces', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbSecret s
JOIN tbFolder f ON s.FolderID = f.FolderID
...
UNION ALL
SELECT 'Folders Without Owners', CAST(COUNT(*) AS NVARCHAR(50)), ''
FROM tbfolder f
