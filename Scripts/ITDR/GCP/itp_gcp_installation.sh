...
export PROJECT=test-onboarding-haim-0
export BUCKET_NAME=testServiceAccountBucket
export SERVICE_ACCOUNT_NAME=testServiceAccount
...
check_required_vars() {
  ...
  ORGANIZATION=$(gcloud projects get-ancestors $PROJECT --format=json | jq -r '.[] | select(.type=="organization") | .id')
...
create_bucket() {
  ...
  # Check if the bucket exists
    if gcloud storage buckets describe --project="$PROJECT" "$FULL_BUCKET_NAME" > /dev/null 2>&1; then
       ...
       if gcloud storage buckets create --project="$PROJECT" "$FULL_BUCKET_NAME"; then
...
enable_apis() {
  # Check if the Cloud Asset API is already enabled
  APIS=("recommender.googleapis.com" "policyanalyzer.googleapis.com" "cloudasset.googleapis.com" "cloudresourcemanager.googleapis.com" "logging.googleapis.com" "admin.googleapis.com" "iam.googleapis.com")
...
create_service_account() {
  ...
  ROLES=("roles/iam.serviceAccountViewer" "roles/policyanalyzer.activityAnalysisViewer" "roles/cloudasset.viewer" "roles/storage.objectCreator" "roles/logging.viewer" "roles/recommender.iamViewer" "roles/resourcemanager.organizationViewer" "roles/iam.organizationRoleViewer")
